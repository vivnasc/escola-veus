...
def write_md():
    ...
    out("- **1 faixa Ancient Ground do véu** (instrumental Loranne, looped) — reciclada pelos 4 ciclos")
    ...
    out("---")
    out("")
    out("## Trilhas Ancient Ground — 1 faixa por véu, reciclada")
    out("")
    out("Catálogo de 100 faixas instrumentais Loranne (álbum *Ancient Ground*) em Supabase.")
    out("Cada véu usa **uma** faixa do catálogo, repetida ao longo dos 4 ciclos do mês —")
    out("recorrência intencional para criar reconhecimento sonoro semanal.")
    out("")
    out("Atribuição inicial (edita `tools/contos-mensais/content.py:AG_TRACKS_BY_VEU` para trocar):")
    out("")
    out("| Véu | Capítulos | Faixa | URL |")
    out("|---|---|---|---|")
    ...
    for veu in order:
        ...
        faixa = content.ag_track_label(veu)
        url = content.ag_track_url(veu)
        out(f"| **{veu}** | {caps_str} | `{faixa}` | <{url}> |")
    out("")
    out("O renderer (`render-slideshow.py`) usa estes URLs diretamente — FFmpeg lê do Supabase.")
    out("Para correr offline, descarrega para `assets/trinta-manhas/musica/veu-{nome}.mp3` e o renderer prefere o local.")
    out("")
    out("---")
    out("")
    ...
    out("")
    out("# 2. Música: as 8 faixas já existem em Supabase (Ancient Ground).")
    out("#    O renderer puxa do URL automaticamente. Para correr offline,")
    out("#    descarregar de cada URL para assets/trinta-manhas/musica/veu-{nome}.mp3.")
    out("")
